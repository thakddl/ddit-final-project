...
public class WorkLogServiceImpl implements WorkLogService {
	...
	public void regist(WorkVO newWork, WorkVO oldWork) throws Exception {
		...
		if (!newWokerStr.equals(oldWokerStr)) {
			workLog.setWhat("담당자");
			String oldUserNm = workLogDAO.selectUserNmById(oldWokerStr);
			String newUserNm = workLogDAO.selectUserNmById(newWokerStr);
			workLog.setBefore(oldUserNm);
			workLog.setAfter(newUserNm);
			workLogDAO.insertWorkLog(workLog);
		}
