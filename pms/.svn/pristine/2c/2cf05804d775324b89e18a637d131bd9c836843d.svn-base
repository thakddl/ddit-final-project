...
<%-- 						<input type="text" class="form-control float-right" id="meetDe" name="meetDe" value="${cri.meetDe }">  --%>
		...
		<div class="col-3">
			<div class="card card-outline card-secondary" style="max-height: 680px; overflow: auto;">
					...
					<c:if test="${!empty confirmList }" >	
						<c:forEach items="${confirmList }" var="conf" varStatus="index">
							<div class="info-box">
								...
								<div class="info-box-content"><a href="detail.do?from=list&prcdNo=${conf.prcdNo}">
									<span class="info-box-text">${conf.title }</span>
									<span class="info-box-number">${conf.meetDe.split(" ")[0] }</span>
								</a></div>
							</div>
						</c:forEach>
					</c:if>
